...
program first
        ...
        use modulos
        implicit none
        integer,parameter::halos=87780-19, box=500, cell=50, nthread=5
...
open(10,file='halosprop_2rvir_2_parallel.dat',status='unknown')

!call OMP_set_num_threads(nthread)   !si no llamo esta subrutina la compu
!$OMP PARALLEL DEFAULT (NONE) &
!$OMP PRIVATE (x,y,z,velx,vely,velz,bx,by,bz,part_td,part_st,part_dm,part_gs, &
!$OMP maxhsml,ssfr,rho,L_dm,L_st,masa_dm, masa_gs,masa_st,spin,np_hl) &        
!$OMP SHARED (pos_hl,vel_hl,abin,ntid,ngas,ndm,nst,tot_td,tot_dm,tot_gs,tot_st, &
!$OMP head_td,head_gs,head_dm,head_st,pos_td,pos_dm,pos_gs,pos_st,vel_td,vel_dm, &
!$OMP vel_st,vel_gs,link_td,link_st,link_dm,link_gs,rvir,dens,sfr,hsml,mass_gs, &
!$OMP mass_dm,mass_st) 
       ...
       !         if (part_td>0) cycle
             !-----------------------------------
                ...
                if (part_dm>25) then
                        ...
                        write(10,*) x,y,z,rvir(i),np_hl(i),part_gs,part_dm,part_st,masa_st,ssfr,spin(i),rho,maxhsml
                endif
              !  endif
        enddo
      !$OMP END DO
