...
program first
        use modulos
        implicit none
        integer,parameter::halos=71417-19, box=500, cell=250
...
write(*,*) 'MAXIMO RADIO VIRIAL:',rmax*1e-3,'Mpc'
      ...
      ! stop 
        ...
        write(*,*) 'NO LiNKEANDO TIDALES (NO LAS MIRO)'
...
open(10,file='halosprop_2rvir_3.dat',status='unknown')
        do i=1,halos                            
                x = pos_hl(1,i)
                y = pos_hl(2,i)
                z = pos_hl(3,i)

                if (sqrt((x-250)**2+(y-250)**2+(z-250)**2)>40) cycle
             ...
             !-----------------------------------
                ...
                call cuentasgas(x,y,z,velx,vely,velz,bx,by,bz,part_gs,ngas,cell,tot_gs, &
                                head_gs,pos_gs,vel_gs,link_gs,rvir(i),dens,mass_gs,sfr,masa_gs,rho,ssfr,hsml) 
                part_dm = 0 
                ...
                if (part_dm>25) then
                        ...
                        write(10,*) x,y,z,rvir(i),part_gs,part_dm,part_st,masa_st,ssfr,rho
                endif
              !  endif
        enddo
       close(10) 
                  ...
                  do l = bz-1,bz+1
                        ...
                        do j = 1,tot(k,m,l)
                                ...
                                if (sqrt(rx**2+ry**2+rz**2)<2*rvir*1e-3) then
                                        ...
                                        o = o + 1
                                        if (hsml(p) > rvir) then
                                                masa = masa + dens(p)*(4./3.)*3.14*rvir
                                        else
                                                masa = masa + dens(p)*(4./3.)*3.14*hsml(p)
                                        endif
                                endif
                               p = link(p)       
                        enddo
