...
program cicjack_omp
 ...
 ! logical :: ilogic
  ...
  call reader()
  dbox = 500000     !al parecer, el dbox es el tamaño del box en Kpc
  parbox = nall(0) 
  print*,'parbox',parbox
  print*,'dbox  ',dbox
...
!            nes(l) = 0
  ...
  !-------------------------------------------------------------------------------------------
      ...
      !$OMP PRIVATE(distlog,ibin,rad,rv,dens_h_pix,bfld_pix, &
      !$OMP xc,yc,zc,ix,iy,iz,iix,iiy,iiz,it,bb,rr,dc,dist,fac,xp,yp,zp,v,ipx,dens_pix   ) &
      !$OMP SHARED (lrmin_vu,xe,ye,ze,grid_pixels,lirst,ll,x,y,z,b2,rho,hsml,dens2, &
      !$OMP npix,facbin,pos,l,pi,dbindist_vu)
                    ...
                    seg: do iiy = iy - 1,iy + 1
                      ter: do iiz = iz - 1,iz + 1
                        ...
                        if(it /= 0) then
                          vecino: do
                            it = ll(it)
                            xp =pos(1,it)*1e3 ; yp=pos(2,it)*1e3
                            zp=pos(3,it)*1e3
                        !    bb = b2(it)
                            rr = rho(it)*1e-9
                            dc = (xc-xp)**2+(yc-yp)**2+(zc-zp)**2
                            dist = sqrt(dc)
                            if(dist < Hsml(it)*1e3)then
                               call KernelGadget(dist, hsml(it)*1e3,fac) 
...
print*, 'aloha'
               dens2  (l,ibin) = sum(  dens_pix)/npix*4*pi           
            enddo radio
      !$OMP END DO
      !$OMP BARRIER
      !$OMP END PARALLEL 
