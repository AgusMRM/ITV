       ...
       ! endif
        ...
        do i=1,nall(0)
                ...
                te(i)=(5./3.-1.)*u(i)*vv*mu*mp/kcgs
                        if (te(i) <= 10**(5) .and. rho(i) < 116 .and. d(i) > rmin .and. d(i)<= rmax) then
                                bin = int(log10(d(i)-rmin)/abin) + 1 
                                p_difu(bin) = p_difu(bin) + 1
                        elseif (te(i) <= 10**(5) .and. rho(i) >= 116 .and. d(i) > rmin .and. d(i)<= rmax) then
                                bin = int(log10(d(i)-rmin)/abin) + 1 
        ...
        do i =1,bines
              ...
              !  vol = (4./3.)*pi*(10**(i*abin)**3)
                ...
                write(10,*) 10**((i-.5)*abin), p_difu(i)/vol, p_cond(i)/vol, p_whim(i)/vol, p_hot(i)/vol, &
                        sqrt(real(p_difu(i)))/vol, sqrt(real(p_cond(i)))/vol, sqrt(real(p_whim(i)))/vol, &
                        sqrt(real(p_hot(i)))/vol
                r0= 10**(i*abin)
        enddo
        close(10)
