program grid_mesh
        ...
        use OMP_lib
        implicit none
        integer, parameter :: cell=50, vec=5,vec2=50, realjack=10
      !  integer,parameter  :: nd=2000,bines=20,halos=10682-19 !12317-19!
      !  integer,parameter  :: nd=2000,bines=20,halos=10682-19 !12317-19!
        integer,parameter  :: nd=2000,bines=30,halos=850366-16 !12317-19!
        real, parameter :: box=500,pi=acos(-1.), rmax=155, rmin=0! elegir el lado del box que voy a armar
        real*8, parameter :: distmin=.1, distmax=10
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!       
 ...
 ! binpart_dm=0; binpart_gs=0; binpart_st=0; binpart_hl=0
  ...
  do ijack=1,realjack 
   ...
   !          bines,abin0,partbin_dm,realjack,jack_dm,ijack)!,distance)
      call correlacion(halos,pos_hl,vec2,trazers_hl,pos_hl2,abin,cell,box,tot_hl,head_hl,link_hl,distmin,distmax, &
 ...
 !##########################################################################33   
    ...
    dpares_hl=( trazers*real(halos)/((real(box))**3) ) * (real(realjack-1)/real(realjack))
    do i=1,bines
...
subroutine correlacion(npart,pos2,vec,n,pos1,abin,cell,box,tot,head,link,distmin,distmax,bines,abin0,partbin,&
        ...
        real*8 :: distmin,distmax,lmin,lmax,abin0,cont,cont0
        cand = 0
        l = 0 
        lmin=log10(distmin)
        lmax=log10(distmax)
  ...
  !$OMP cand,dist,l,x0,y0,z0,dx,dy,dz,r,vec0,o,q,ibin,cont,cont0) &
                ...
                do j= bx-1,bx+1
                 ...
                 do m= bz-1,bz+1
                    ...
                    do u=1,tot(bxx,byy,bzz)
                        ...
                        r=log10(r)
                       if (r>lmin .and. r<lmax ) cont0 = cont0 + 1
                       if (r>lmin .and. r<lmax .and. &
                               jack(p) /= ijack ) then
