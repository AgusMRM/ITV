...
program grid_mesh
        ...
        use OMP_lib
        implicit none
        integer, parameter :: cell=100, vec=5,vec2=50 ,nd=2000,halos=80906!87780-19
        real, parameter :: box=200, rmax=15.0, rmin=0.0! elegir el lado del box que voy a armar
 ...
 !zc=441.538681-zbox+250-150
        ...
        allocate(pos_gs(3,nall(0)), pos_dm(3,nall(1)), pos_st(3,halos))
        do i=1,nall(0)
       ...
       !open(10,file='/mnt/is2/dpaz/ITV/R1198/halos/halos_50.ascii',action='read',status='unknown')
       open(10,file='/home/arodriguez/Void-s-Gastrophysics/PostSemi/halosprop_R.dat',action='read',status='unknown')
       !do i=1,19
       ! read(10,*) 
       ! enddo
      estrellas=0 
        do i=1,halos 
        read(10,*)    aa,pos_st(1,i),pos_st(2,i),pos_st(3,i),aa,aa,aa,aa,estrellas(i)
        pos_st(1,i)=pos_st(1,i)-150
        pos_st(2,i)=pos_st(2,i)-150
        pos_st(3,i)=pos_st(3,i)-150
        enddo   
        deallocate(pos,vel)
 close(10)
        ...
        do i=1,halos
                ...
                if (d<rmax .and. d>rmin .and. estrellas(i)>0) then
                        k=k+1
                endif
        enddo
        ...
        call linkedlist(halos,abin,cell,pos_st,head_st,tot_st,link_st)
     !************************************************************   
