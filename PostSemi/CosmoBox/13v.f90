program grid_mesh
        ...
        use OMP_lib
        implicit none
        integer, parameter :: cell=120, vec=10
    ...
    !do i=1,nst
       call vecina(vec,nst,pos_st,abin,cell,box,tot_st,head_st,link_st)!,distance)
       
    !enddo
    close(12)
    ! deallocate(distance)
endprogram grid_mesh
subroutine vecina(vec,n,pos,abin,cell,box,tot,head,link)!,d)
        implicit none
        integer :: i,j,k,m,cand,bxx,byy,bzz,u,p,l
        integer :: bx,by,bz,n,cell,vec
  ...
  !$OMP cand,dist,l,x0,y0,z0,dx,dy,dz,r) &
  !$OMP SHARED (pos,abin,tot,head,link,n,cell,vec,box )
