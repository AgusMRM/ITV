program grid_mesh
        ...
        use OMP_lib
        implicit none
        integer, parameter :: cell=500, vec=5,vec2=50, realjack=100
        integer,parameter  :: nd=2000,bines=20,halos=71417-19 !12317-19!
        !integer,parameter  :: nd=2000,bines=20,halos=10682-19 !12317-19!
        real, parameter :: box=500,pi=acos(-1.), rmax=30, rmin=25! elegir el lado del box que voy a armar
        real*8, parameter :: distmin=.001, distmax=1
        real,parameter :: boxt=125 
        integer*8,parameter :: estrellas=10439184 , darkmatter=134217728 , &
                            gas=134217728 - 10439184
        integer,parameter :: nthread=40  
        ...
        real :: dpares_st, dpares_gs, dpares_dm
        ...
        integer :: grid,ibin
        integer*8,dimension(bines,realjack) :: pares_st, pares_gs, pares_dm
        integer*8,dimension(bines,realjack) :: pares_hl
       ! integer*8,dimension(bines,realjack) :: partbin_st, partbin_gs, partbin_dm
       ! integer*8,dimension(bines,realjack) :: partbin_hl
        integer*8,allocatable,dimension(:,:,:) :: partbin_st, partbin_gs, partbin_dm
        integer*8,allocatable,dimension(:,:,:) :: partbin_hl
        real*8 ::abin0,radio,radio0
        integer*8,allocatable :: binpart(:,:)
        integer, allocatable,dimension(:)  :: jack_dm, jack_gs, jack_st, jack_hl
        real,dimension(bines) :: pares_teoricos_st, pares_teoricos_dm,pares_teoricos_gs
        real,dimension(bines) :: u_st,u_dm,u_gs, var_st,var_dm,var_gs
        real,dimension(bines,realjack) :: xi_st,xi_dm,xi_gs
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!      
...
open(123,file='/mnt/is2/dpaz/ITV/S1373/halos/halos_50.ascii',status='unknown')
!open(123,file='/home/arodriguez/halosprueba/halos_0.0.ascii',status='unknown')
!open(123,file='/home/arodriguez/halosprueba/galaxias_S.ascii',status='unknown')
        do i=1,19
        read(123,*)
        enddo
        do i=1,halos
 ...
 ! binpart_dm=0; binpart_gs=0; binpart_st=0; binpart_hl=0
    trazers = trazers_st
  allocate(partbin_dm(trazers,bines,realjack),partbin_gs(trazers,bines,realjack) & 
       ,partbin_st(trazers,bines,realjack),partbin_hl(trazers,bines,realjack)) 
  partbin_gs=0; partbin_dm=0; partbin_st=0; partbin_hl=0 

  call OMP_set_num_threads(nthread)   
!  do ijack=1,realjack 
   write(*,*) 'realizacion:', ijack
      call correlacion(nst,pos_st,vec,trazers,pos_st2,abin,cell,box,tot_st,head_st,link_st,distmin,distmax, &
         bines,abin0,partbin_st,realjack,jack_st)!,distance)
      call correlacion(ngs,pos_gs,vec2,trazers,pos_st2,abin,cell,box,tot_gs,head_gs,link_gs,distmin,distmax, &
          bines,abin0,partbin_gs,realjack,jack_gs)!,distance)

      call correlacion(ndm,pos_dm,vec2,trazers,pos_st2,abin,cell,box,tot_dm,head_dm,link_dm,distmin,distmax, &
             bines,abin0,partbin_dm,realjack,jack_dm)!,distance)
   !   call correlacion(ndm,pos_dm,vec2,trazers_hl,pos_hl2,abin,cell,box,tot_dm,head_dm,link_dm,distmin,distmax, &
   !        bines,abin0,partbin_hl,realjack,jack_dm,ijack)!,distance)
!   enddo   
 !##########################################################################33   
 !##########################################################################33   
 !##########################################################################33   
    dpares_st=(real(realjack-1)/real(realjack)) *trazers*estrellas/(boxt**3) 
    dpares_gs=(real(realjack-1)/real(realjack)) *trazers*darkmatter/(boxt**3) 
    dpares_dm=(real(realjack-1)/real(realjack)) *trazers*gas/(boxt**3) 
    radio0=distmin    
    do i=1,bines
    ...
    do j=1,realjack
    do k=1,trazers
        pares_dm(i,j) = pares_dm(i,j) + partbin_dm(k,i,j)
        pares_gs(i,j) = pares_gs(i,j) + partbin_gs(k,i,j)
        pares_st(i,j) = pares_st(i,j) + partbin_st(k,i,j)
        pares_hl(i,j) = pares_hl(i,j) + partbin_hl(k,i,j)
    enddo
    enddo
    enddo
    do j=1,realjack
        do i=1,bines
                xi_st(i,j)=pares_st(i,j)/pares_teoricos_st(i) - 1
                xi_gs(i,j)=pares_gs(i,j)/pares_teoricos_gs(i) - 1
                xi_dm(i,j)=pares_dm(i,j)/pares_teoricos_dm(i) - 1
...
!                               partbin_hl(i,2)
        write(12,*) radio, u_st(i), u_dm(i), u_gs(i), var_st(i), var_dm(i), var_gs(i)
...
subroutine correlacion(npart,pos2,vec,n,pos1,abin,cell,box,tot,head,link,distmin,distmax,bines,abin0,partbin,&
        ...
        integer :: ibin,bines
        integer*8 ::pares
        integer*8,dimension(n,bines,realjack) :: partbin
  ...
  !$OMP cand,dist,l,x0,y0,z0,dx,dy,dz,r,vec0,o,q,ibin,ii) &
  !$OMP SHARED (pos1,abin,tot,head,link,n,cell,vec,box,nd,pos2,lmin , &
  !$OMP lmax,bines,partbin,abin0,distmin,distmax,binpart,ijack,jack,realjack)
                 ...
                 do m= bz-1,bz+1
                    ...
                    do u=1,tot(bxx,byy,bzz)
                       ...
                       if (r>lmin .and. r<lmax) then
                        ...
                        do ii=1,realjack 
                        if (ii /= ijack) partbin(i,ibin,ii) = partbin(i,ibin,ii) + 1
                        enddo 
                       endif 
                        p = link(p)
                    enddo
