...
program cicjack_omp
 ...
 ! real(DP),parameter              :: rmin=1000.
  ...
  integer,parameter           :: nbindist  =200
  ...
  integer :: iHsml
  integer :: npix
  real :: rhomin, rhomax,rad0,promedio,prom,densidad,dens3,error,temp 
 ...
 print*, ibin 
                        ...
                        !    bb = b2(it)
                            ...
                            if(dist < Hsml(it))then
                               call KernelGadget(dist, hsml(it),fac)
                                ...
                                if (temp<10**5.5 .and. rho(it)<1.8) then
                                dens_pix1(ipx) = dens_pix1(ipx) +mass(it)*fac
                                elseif (temp<10**5.5 .and. rho(it)>=1.8) then
                                dens_pix2(ipx) = dens_pix2(ipx) +mass(it)*fac
                                elseif (temp>=10**5.5 .and. rho(it)<1.8) then
                                dens_pix3(ipx) = dens_pix3(ipx) +mass(it)*fac
                                elseif (temp>=10**5.5 .and. rho(it)>=1.8) then
                                dens_pix4(ipx) = dens_pix4(ipx) +mass(it)*fac
                                endif
               ...
               dens2(l,ibin) = sum(dens_pix)/npix   
                do ijack=1,npix
                        ilogic=.TRUE.
                        ilogic(ijack)=.FALSE.
                        dens_jack(ibin,ijack) = sum(dens_pix,mask=ilogic)/npix
                enddo
            enddo radio
      !$OMP END DO
      !$OMP BARRIER
...
jmed=0.0; jsig=0.0; sig=0.0 
  do i=1,nbindist
              ...
              !  jsig=0.0
                ...
                do ijack=1,npix
                jmed(i)=jmed(i)+dens_jack(i,ijack)
                enddo
                jmed(i) = jmed(i)/npix
                
                do ijack=1,npix
                jsig(i)=jsig(i)+(dens_jack(i,ijack)-jmed(i))**2
                enddo
                sig(i)=sqrt((47./48.)*jsig(i))
!                write(10,*) rad, dens2(1,i),jmed(i),sig(i)
 enddo
 ...
 do i=1,nbindist
        ...
        distlog = (i+ 0.5)*dbindist_vu + lrmin_vu
        rad    = (10.0**distlog)         !*rv
        densidad = densidad + dens2(1,i)*rad
        o = o + 1

        error = error + jsig(i)**2
        if (mod(i,10)==0) then
                !distlog = (i - 0.5)*dbindist_vu + lrmin_vu
                !rad    = (10.0**distlog)         !*rv
                promedio = 1./(rad-rad0)*densidad*log(10.)*(log10(rad)-log10(rad0))/real(o)
                error = sqrt(error)/real(o)
                rad0 = rad 
                densidad = 0   
                o = 0             
        write(10,*) rad, promedio, 999,error
                error = 0
        endif
 enddo
 close(10)
